...
module.exports = function (old, callback) {
  ...
  db.find({date: date, time: time, description: tagUtil.removeTags(description)}, function (err, registerDuplicate) {
      ...
      db.find({date: date}).sort({ id: -1 }).limit(1).exec(function (err, registers) {
        ...
        if (time && description) {
          var register = {
            ...
            datetime: datetime,
            time: time,
            description: tagUtil.removeTags(description),
            tags: tagUtil.getTags(description)
          }
          db.insert(register, function (err, newDoc) {
            if (err) return callback(err)
            console.log()
            console.log(' ', ' OK '.bgGreen, 'Registry successfully saved!')
            integration.checkAndSend(register, function (err) {
              if (err) return callback(err)
              console.log()
              callback()
            })
